                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-center gap-8 lg:gap-16">
                        ...
                        <div className="flex flex-col gap-4 w-full lg:w-1/4">
                            {["Shopify", "Mercado Libre", "Tu Propio Sistema"].map((platform) => (
                                <div key={platform} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 text-center font-medium text-gray-700 relative group">
                                    {platform}
                                    {/* Connector Line (Mobile: Down, Desktop: Right) */}
                                    <div className="hidden lg:block absolute right-[-2rem] top-1/2 w-[2rem] h-[2px] bg-gray-300"></div>
                                    <div className="lg:hidden absolute bottom-[-1rem] left-1/2 h-[1rem] w-[2px] bg-gray-300"></div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="w-full lg:w-1/3 relative z-10">
                            <div className="bg-white p-8 rounded-2xl shadow-2xl border-2 border-primary text-center relative overflow-hidden">
                        ...
                        <div className="flex flex-col gap-3 w-full lg:w-1/4">
                            {["UES", "Distrilogic", "Correo Uruguayo", "Soy Delivery", "Otros"].map((carrier, index) => (
                                <div key={carrier} className="bg-white p-3 rounded-lg shadow-sm border border-gray-100 text-center text-sm font-medium text-gray-600 relative">
                                    {/* Connector Line (Desktop Only - Left side of box) */}
                                    <div className="hidden lg:block absolute left-[-2rem] top-1/2 w-[2rem] h-[2px] bg-gray-300 transform -translate-y-1/2"></div>
                                    {carrier}
                                </div>
                            ))}
                        </div>
